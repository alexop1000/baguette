...
class Modlog:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.settings = js.load("data/modlog/settings.json")

    @commands.group(pass_context=True, no_pm=True)
    async def modlogset(self, ctx):
        if ctx.message.author.id == "289463060178010123" or ctx.message.author.id == "278830391090216960":
            ...
            self.bot.say("You are not authorized to do that!")
    ...
    def save_settings(self):
        js.dump(self.settings, "data/modlog/settings.json", overwrite=True)
...
def check_files():
    if not os.path.exists("data/modlog/settings.json"):
        ...
        js.dump({}, "data/modlog/settings.json")
